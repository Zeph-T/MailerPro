...
export class Controller {
    ...
    create(req, res) {
        isAuthenticated(req, res, () => {
            ...
            let createdTag = new Tag(createdTagData);
            createdTag.save()
    ...
    addTagToContact(req,res){
        isAuthenticated(req,res , ()=> {
            ...
            Contact.findOneAndUpdate({_id : mongoose.Types.ObjectId(contactId)},{$push : {tags : tagId}},{new : true}).then(response=>
    ...
    deleteTag(req,res){
        isAuthenticated(req,res , ()=> {
            let tagId = req.body.tagId;
            Contact.updateMany({tags : {$in : tagId}},{$pull : {tags : tagId}}).then(()=>
                Tag.deleteOne({_id : mongoose.Types.ObjectId(tagId)}).then(response=>
                    res.json({"data" : response})
                ).catch(err=>res.json({"data" : {error : err}}))
            ).catch(err=> res.json({"data" : { error : err }}))
        })      
