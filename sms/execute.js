...
let runSMSCampaign = async function (campaign, contactList) {
    let deferred = Q.defer();
    try{
        let list = new LinkedList();
        let deferred = Q.defer();
        ...
        let smsSendPromise = [];
        let template = await Template.findOne({_id : campaign.template}).lean();
        let templateText = template.content;
...
let executeCampaign = async function (document) {
    try {
        ...
        if (document.campaignId) {
            ...
            let campaign = null;
            if(campaignType === "EMAIL")campaign = await Campaign.findById(campaignId);
            else campaign = await smsCampaign.findById(campaignId);
